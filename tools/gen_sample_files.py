...
SAMPLES = {
    ...
    "dotnet-service": dict(lang="csharp", image="build-dotnet:8.0", fmt="cobertura", team="team-pricing-dotnet",
        rules=["Money is `decimal`, never `double`.",
               "Constructor injection (primary constructors are fine); no static state or service locators.",
               "Nullable reference types on; warnings are errors; max cyclomatic complexity 8 (CA1502)."]),
    "angular-app": dict(lang="typescript", image="build-node:22", fmt="cobertura", team="team-storefront-ng",
        rules=["Business logic lives in services or plain TypeScript, not in templates.",
               "Standalone components, signals and `OnPush` change detection.",
               "Components are tested through TestBed by behaviour (rendered output), not private methods.",
...
EXTRA_OWNERS = {
    ...
    "dotnet-service": "/Directory.Build.props @platform/architects\n/.editorconfig        @platform/architects\n/CodeMetricsConfig.txt @platform/architects\n",
    "angular-app": "/eslint.config.js     @platform/architects\n",
